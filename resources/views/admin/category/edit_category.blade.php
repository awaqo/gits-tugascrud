...
@section('content')
    <div class="container px-16 py-16">
        ...
        <div class="text-2xl mt-5">Form Edit Kategori</div>
        <div class="max-w-xl">
            <form class="space-y-6" action="{{ url('admin/category/'.$categories->id) }}" method="POST">
                @method('PUT')
                @csrf
                <div>
                    <label for="category_name" class="block mb-3 text-sm font-medium text-gray-900">Nama Kategori</label>
                    <input type="text" name="category_name" id="category_name" value="{{ $categories->name }}" 
